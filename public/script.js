    ...
    async function loadPendingApplications() {
        ...
        try {
            ...
            querySnapshot.forEach(docSnapshot => {
                ...
                row.innerHTML = `
          <td class="px-6 py-4 whitespace-nowrap"><input type="checkbox" class="rounded text-blue-600 focus:ring-blue-500"></td>
          ...
          <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm text-gray-900">${data.soloParentIdNumber || 'N/A'}</div></td>
          <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm text-gray-900">${data.createdAt ? data.createdAt.toDate().toLocaleDateString() : 'N/A'}</div></td>
          ...
          <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
            <div class="flex justify-end items-center space-x-3">
              <a href="profile.html?id=${docSnapshot.id}" target="_blank" class="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700">View</a>
...
function initMembersPage() {
    ...
    async function loadVerifiedMembers() {
        ...
        try {
          ...
          <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm text-gray-900">${data.address || 'N/A'}</div></td>
          <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm text-gray-900">${data.soloParentIdNumber || 'N/A'}</div></td>
          <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm text-gray-900">${data.projectReceived || 'N/A'}</div></td>
          ...
          <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
            <div class="flex justify-end space-x-2">
              <a href="profile.html?id=${docSnapshot.id}" class="text-blue-600 hover:text-blue-900" title="View Details"><i data-feather="eye" class="w-4 h-4"></i></a>
              <a href="#" class="text-blue-600 hover:text-blue-900" title="Edit Member"><i data-feather="edit" class="w-4 h-4"></i></a>
            </div>
          </td>`;
...
function initCategoriesPage() {
    ...
    async function loadAllData() {
        ...
        try {
            ...
            allApprovedMembers.forEach(doc => {
                ...
                if (description && categoryCounts[description] !== undefined) categoryCounts[description]++;
                const location = data.municipality || data.placeOfBirth;
                if (municipalityCounts[location] !== undefined) municipalityCounts[location]++;
            });
    ...
    function renderCategoryFilters(counts) {
      ...
      <a href="#" data-category="All" class="flex justify-between items-center px-3 py-2 rounded-md bg-blue-50 text-blue-700 category-link">
        ...
        <span class="text-xs font-medium">${allApprovedMembers.length}</span>
      </a>`;
        ...
        <a href="#" data-category="${description}" class="flex justify-between items-center px-3 py-2 rounded-md hover:bg-gray-50 category-link">
          <span class="flex items-start"><span class="text-xs font-mono w-8 text-gray-400 flex-shrink-0">${code}</span><span class="ml-1">${description}</span></span>
    ...
    function renderMemberProfiles(members) {
        ...
        <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-100 hover:border-blue-200 transition">
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1"><h3 class="font-semibold text-gray-800">${name}</h3><p class="text-sm text-gray-500">ID: ${data.soloParentIdNumber || 'N/A'}</p><p class="text-sm text-gray-500 mt-1">${data.address || 'N/A'}</p></div>
          </div>
          <div class="mt-4"><a href="profile.html?id=${doc.id}" class="w-full block text-center py-2 px-4 border border-blue-600 text-blue-600 font-medium rounded-md hover:bg-blue-50 transition">View Profile</a></div>
        </div>`;
        });
    }
...
function initEventsPage() {
            ...
            <div class="flex justify-between items-start">
              <div class="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p class="text-sm text-gray-500 mt-1">${formattedDate} • ${event.eventTime}</p>
              ...
              <div class="flex gap-2">
                <button data-id="${doc.id}" class="edit-event-btn text-blue-600 hover:text-blue-800 p-2 rounded-full hover:bg-blue-50"><i data-feather="edit" class="w-4 h-4 pointer-events-none"></i></button>
                <button data-id="${doc.id}" class="delete-event-btn text-red-600 hover:text-red-800 p-2 rounded-full hover:bg-red-50"><i data-feather="trash-2" class="w-4 h-4 pointer-events-none"></i></button>
              </div>
            </div>
          </div>`;
            });
    ...
    function addDynamicButtonListeners() {
        ...
        eventListEl.querySelectorAll('.edit-event-btn').forEach(btn => {
            btn.addEventListener('click', () => alert("Edit functionality is not yet implemented."));
        });
    }
    loadActiveEvents();
}
...
async function initProfilePage() {
    ...
    try {
        ...
        if (user.status === 'approved') {
            ...
            approveBtn.style.display = 'flex'; rejectBtn.style.display = 'flex';
        } else {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (page.endsWith('dashboard.html')) initDashboardPage();
    else if (page.endsWith('applications.html')) initApplicationsPage();
    else if (page.endsWith('members.html')) initMembersPage();
    else if (page.endsWith('categories.html')) initCategoriesPage();
    else if (page.endsWith('events.html')) initEventsPage();
    else if (page.endsWith('announcements.html')) initAnnouncementsPage();
    else if (page.endsWith('profile.html')) initProfilePage();
    else if (page.endsWith('index.html') || page === '/') handlePasswordToggle();
