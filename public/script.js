...
import { 
    ...
    addDoc, deleteDoc, getDoc // ✅ ADDED getDoc (singular) for profile page
...
// --------------------------------------------------------------------------
// ✅ NEW: GLOBAL HELPER FUNCTIONS (for Approve/Reject)
// --------------------------------------------------------------------------
async function approveUser(docId) {
    // MODIFIED: Do not use confirm()
    try {
...
function handleSidebarHighlight() {
  ...
  sidebarLinks.forEach(link => {
    ...
    // ✅ ADDED: Highlight "Categories" or "Members" when on a profile page
      ...
      try {
        ...
        querySnapshot.forEach(docSnapshot => {
          ...
          row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap"><div class="flex items-center"><div class="ml-4"><div class="text-sm font-medium text-gray-900">${name}</div></div></div></td>
            <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm text-gray-900">${data.email || 'N/A'}</div></td>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
              <div class="flex justify-end items-center space-x-3">
                <!-- ✅ UPDATED to call global functions -->
                ...
                row.innerHTML = `
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex justify-end space-x-2">
                            <!-- ✅ UPDATED link to point to profile.html -->
...
function initCategoriesPage() {
    ...
    const CATEGORY_LIST = [ "Annulment", "Widow / Widower", "Child is Adopted", "Unmarried", "PWD", "OFW", "Other Cases" ];
        ...
        members.forEach(doc => {
            ...
            memberGridEl.innerHTML += `
                <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-100 hover:border-blue-200 transition">
                    ...
                    <div class="mt-4">
                        <!-- ✅ UPDATED link to point to profile.html -->
                        <a href="profile.html?id=${doc.id}" class="w-full block text-center py-2 px-4 border border-blue-600 text-blue-600 font-medium rounded-md hover:bg-blue-50 transition">
                            View Profile
                        </a>
                    </div>
                </div>`;
...
function initEventsPage() {
    ...
    if (eventForm) {
        eventForm.addEventListener('submit', async (e) => {
            ...
            if (!title || !dateValue || !timeValue) {
                console.error("Please fill in at least the Event Title, Date, and Time."); // No alert()
                return;
            }
            try {
                ...
                console.log("✅ Event created successfully!"); // No alert()
                eventForm.reset(); 
                loadActiveEvents(); 
            } catch (error) {
    ...
    async function deleteEvent(eventId) {
        ...
        try {
            ...
            console.log("Event deleted successfully."); // No alert()
            loadActiveEvents(); 
        } catch (error) {
    ...
    function addDynamicButtonListeners() {
        ...
        eventListEl.querySelectorAll('.edit-event-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                console.log("Edit functionality is not yet implemented. Event ID:", e.target.dataset.id); // No alert()
            });
        });
    }
    loadActiveEvents();
...
// --------------------------------------------------------------------------
// ✅ NEW: PROFILE PAGE LOGIC (`profile.html`)
// --------------------------------------------------------------------------
async function initProfilePage() {
    ...
    // ✅ ADDED: Add click listener for printing
    if (printBtn) {
    ...
    try {
        ...
        editBtn.addEventListener('click', () => console.log('Edit functionality is not yet implemented.')); // No alert()
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (isPage('dashboard.html')) {
        ...
        initEventsPage();
    } else if (isPage('profile.html')) { // ✅ ADDED THIS
        initProfilePage();
    } else if (isPage('index.html') || isPage('/') || window.location.pathname === '/') {
        handlePasswordToggle(); // For login page
    }
